...
export const login = (navigation) => async (dispatch, getState) => {
  const {
    Auth: {
      email, password, rememberMe, activationCodeSent
    }
  } = getState();
  try {
...
export const signup = (navigation) => async (dispatch, getState) => {
  ...
  if (!isEmail(email)) { return signUpFormValidation(dispatch, 'Invalid email', 'email'); }
  if (!isLength(password, { min: 6, max: undefined })) { return signUpFormValidation(dispatch, 'Password must be longer than 6 characters', 'password'); }
  if (password.trim() !== retypedPassword.trim()) { return signUpFormValidation(dispatch, 'Passwords don`t match', 'retypedPassword'); }
